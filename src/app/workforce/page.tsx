...
function calcWCS(form: FormData) {
  ...
  const bonus = bonusItems.filter((b) => b.earned).reduce((s, b) => s + b.pts, 0);
  const total = base + bonus;

  const tierIndex =
    total >= 86 ? 3 : total >= 71 ? 2 : total >= 51 ? 1 : 0;

  return { base, bonus, total, expPts, scalePts, tourPts, bonusItems, tier: TIER_CONFIG[tierIndex] };
}
...
function Label({ htmlFor, children, optional }: { htmlFor?: string; children: React.ReactNode; optional?: boolean }) {
  return (
    <label htmlFor={htmlFor} className="block text-sm font-semibold text-on-surface-variant mb-1.5">
      {children}
      {optional && <span className="ml-1 font-normal italic text-xs">(optional)</span>}
    </label>
  );
}
      ...
      {options.map((opt) => {
        ...
        return (
          ...
          >
            ...
            >
              {checked && <span className="w-2.5 h-2.5 rounded-full bg-[#FF5A30] block" />}
            </span>
            {opt}
          </button>
        );
...
function ScoreGauge({ score, max = 145 }: { score: number; max?: number }) {
  ...
  const tier = TIER_CONFIG[score >= 86 ? 3 : score >= 71 ? 2 : score >= 51 ? 1 : 0];
    ...
    <div className="text-center">
      <div className="relative inline-flex flex-col items-center">
        <div
          ...
          style={{ background: `conic-gradient(${pct >= 86 ? "#f59e0b" : pct >= 71 ? "#3b82f6" : pct >= 51 ? "#10b981" : "#94a3b8"} ${pct * 3.6}deg, #e2e8f0 0deg)` }}
        >
          <div className="w-24 h-24 rounded-full bg-surface-container-lowest flex flex-col items-center justify-center">
            <span className="text-4xl font-black text-on-surface leading-none">{score}</span>
            <span className="text-xs text-on-surface-variant font-semibold">/ {max}</span>
          </div>
        </div>
        <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold ${tier.bg} ${tier.text}`}>
          <span className="material-symbols-outlined text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>
            {score >= 86 ? "military_tech" : score >= 71 ? "verified" : score >= 51 ? "badge" : "person"}
          </span>
          ...
          <div className="text-center max-w-lg">
            ...
            <div className="mt-8 mb-4">
              ...
              <p className="text-on-surface-variant">
                Your WCS score has been calculated. The Ckrowd team will verify your credentials
                and confirm your tier within 48 hours via WhatsApp or email.
              </p>
            </div>

            <div className={`rounded-2xl p-5 border mb-6 text-left ${tier.bg} ${tier.border}`}>
              <div className="flex items-center gap-2 mb-3">
                <span className="material-symbols-outlined text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>
                  {wcs.total >= 86 ? "military_tech" : wcs.total >= 71 ? "verified" : wcs.total >= 51 ? "badge" : "person"}
                </span>
                <h4 className={`text-sm font-bold ${tier.text}`}>Tier {tier.tier} — {tier.label}</h4>
              </div>
              <ul className="space-y-1.5">
                {tier.perks.map((p) => (
                  <li key={p} className={`flex items-start gap-2 text-xs ${tier.text}`}>
                    <span className="material-symbols-outlined text-xs mt-0.5" style={{ fontVariationSettings: "'FILL' 1" }}>check_circle</span>
                    {p}
                  </li>
                ))}
              </ul>
              {tier.aya && (
                <div className="mt-3 pt-3 border-t border-current/20 flex items-center gap-2">
                  ...
                  <span className={`text-xs font-bold ${tier.text}`}>Aya Nakamura Pilot: {tier.aya}</span>
                </div>
              )}
            </div>
          ...
          <header className="mb-10">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-on-surface-variant">
                  Register as a verified CTaaS crew member. Your answers are scored
                  instantly to determine your Workforce Capability Score and tier placement.
                </p>
              </div>
              {/* Live score pill (visible from step 1 onwards) */}
              {step > 0 && (
                <div className={`shrink-0 flex flex-col items-center px-5 py-3 rounded-2xl border-2 ${tier.bg} ${tier.border} min-w-24`}>
                  <span className={`text-3xl font-black ${tier.text}`}>{wcs.total}</span>
                  <span className={`text-[10px] font-bold uppercase tracking-wider ${tier.text} opacity-70`}>
                    WCS
                  </span>
                  <span className={`text-[10px] font-bold ${tier.text} mt-0.5`}>
                    Tier {tier.tier}
                  </span>
                </div>
              )}
            ...
            <div className="mt-4 flex items-center gap-2 text-xs text-on-surface-variant">
              ...
              Estimated completion: 3 minutes &nbsp;·&nbsp; Submitted → Scored → Tiered
            </div>
          </header>
            ...
            >
              ...
              {step === 0 && (
                <div className="space-y-8">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="material-symbols-outlined text-[#FF5A30]">person</span>
                    <h3 className="text-xl font-bold font-(family-name:--font-manrope)">Personal Information</h3>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <Label htmlFor="full-name">Full Name</Label>
                      <input id="full-name" type="text" placeholder="e.g. Kofi Mensah" className={inputClass} required
                        value={form.fullName} onChange={(e) => set("fullName", e.target.value)} />
                    </div>
                    <div>
                      <Label htmlFor="pref-name" optional>Preferred Name / Stage Name</Label>
                      <input id="pref-name" type="text" placeholder="e.g. DJ Phantom" className={inputClass}
                        value={form.preferredName} onChange={(e) => set("preferredName", e.target.value)} />
                    </div>
                    <div>
                      <Label htmlFor="phone">Phone Number</Label>
                      <input id="phone" type="tel" placeholder="+234 800 000 0000" className={inputClass} required
                        value={form.phone} onChange={(e) => set("phone", e.target.value)} />
                    </div>
                    <div>
                      <Label htmlFor="whatsapp" optional>WhatsApp Number (if different)</Label>
                      <input id="whatsapp" type="tel" placeholder="+233 20 000 0000" className={inputClass}
                        value={form.whatsapp} onChange={(e) => set("whatsapp", e.target.value)} />
                    </div>
                    <div className="md:col-span-2">
                      <Label htmlFor="email">Email Address</Label>
                      <input id="email" type="email" placeholder="yourname@email.com" className={inputClass} required
                        value={form.email} onChange={(e) => set("email", e.target.value)} />
                    </div>
                    <div>
                      <Label htmlFor="city">City / Base Location</Label>
                      <input id="city" type="text" placeholder="e.g. Lagos" className={inputClass} required
                        value={form.cityBase} onChange={(e) => set("cityBase", e.target.value)} />
                    </div>
                    <div>
                      <Label htmlFor="country">Country</Label>
                      <input id="country" type="text" placeholder="e.g. Nigeria" className={inputClass} required
                        value={form.country} onChange={(e) => set("country", e.target.value)} />
                    </div>
                    <div>
                      <Label htmlFor="nationality">Nationality</Label>
                      <input id="nationality" type="text" placeholder="e.g. Ghanaian" className={inputClass} required
                        value={form.nationality} onChange={(e) => set("nationality", e.target.value)} />
                    </div>
                    <div>
                      <Label htmlFor="national-id" optional>National ID / Passport Number</Label>
                      <input id="national-id" type="text" placeholder="Document number only" className={inputClass}
                        value={form.nationalId} onChange={(e) => set("nationalId", e.target.value)} />
                      <p className="text-xs text-on-surface-variant mt-1.5 italic">
                        Required for cross-border eligibility — document number only.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {step === 1 && (
                <div className="space-y-10">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="material-symbols-outlined text-[#FF5A30]">engineering</span>
                    <h3 className="text-xl font-bold font-(family-name:--font-manrope)">Role &amp; Specialisation</h3>
                  </div>

                  <div>
                    <Label>Primary Role / Job Title <span className="font-normal italic text-xs">(select all that apply)</span></Label>
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              ...
                              set("roles", checked ? arr.filter((x) => x !== role) : [...arr, role]);
                            }}
                  ...
                  {form.roles.includes("Other") && (
                    <div>
                      ...
                      <input id="other-role" type="text" placeholder="Your role title" className={inputClass}
                        value={form.otherRole} onChange={(e) => set("otherRole", e.target.value)} />
                    </div>
                  )}

                  <div>
                    <Label>Years of Experience in Live Events / Concert Production</Label>
                    <div className="mt-2">
                      <RadioGroup
                  ...
                  <div>
                    <Label>Largest Event You Have Worked (Approximate Attendance)</Label>
                    <div className="mt-2">
                      <RadioGroup
                        options={["Under 500", "500–2,000", "2,001–10,000", "10,001–50,000", "50,000+"]}
              ...
              {step === 2 && (
                <div className="space-y-10">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="material-symbols-outlined text-[#FF5A30]">travel_explore</span>
                    <h3 className="text-xl font-bold font-(family-name:--font-manrope)">Touring Capacity</h3>
                  </div>
                  ...
                  <div>
                    <div className="flex items-start justify-between gap-4 mb-2">
                      <Label>Can you deploy within 48 hours if required?</Label>
                      <span className="text-xs font-bold text-[#FF5A30] bg-[#FF5A30]/10 px-2 py-1 rounded-full shrink-0">
                        +10 bonus pts
                      </span>
                    </div>
                    <RadioGroup
                      options={["Yes", "No", "Sometimes — depends on advance notice"]}
                  ...
                  <div>
                    <div className="flex items-start justify-between gap-4 mb-2">
                      <Label optional>Do you hold a valid travel document (passport) for cross-border work?</Label>
                      <span className="text-xs font-bold text-[#FF5A30] bg-[#FF5A30]/10 px-2 py-1 rounded-full shrink-0">
                        +10 bonus pts
                      </span>
                    </div>
                    <RadioGroup
                      options={["Yes — currently valid", "Yes — renewal pending", "No"]}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <Label htmlFor="markets" optional>Markets / Countries You Have Worked In</Label>
                      <textarea id="markets" rows={3} placeholder="e.g. Nigeria, Ghana, Côte d'Ivoire, Senegal, Kenya..."
                        ...
                        value={form.marketsWorked} onChange={(e) => set("marketsWorked", e.target.value)} />
                    </div>
                    <div>
                      <Label htmlFor="day-rate">Expected Day Rate</Label>
                      <input id="day-rate" type="text" placeholder="e.g. $150/day or ₦80,000/day" className={inputClass} required
                        value={form.dayRate} onChange={(e) => set("dayRate", e.target.value)} />
                      <p className="text-xs text-on-surface-variant mt-1.5 italic">
                        USD or local currency. Locked in your crew profile upon certification.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {step === 3 && (
                <div className="space-y-10">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="material-symbols-outlined text-[#FF5A30]">verified_user</span>
                    <h3 className="text-xl font-bold font-(family-name:--font-manrope)">Credentials &amp; Verification</h3>
                  </div>
                  ...
                  <section className="bg-surface-container-low rounded-2xl p-6 border border-outline-variant/10">
                    <div className="flex items-start justify-between gap-4 mb-4">
                      <div>
                        <h4 className="font-bold text-on-surface mb-1 font-(family-name:--font-manrope)">
                          Reference from a Certified Promoter or Production Company
                        </h4>
                        <p className="text-sm text-on-surface-variant">
                          A verified reference from a CTaaS-certified promoter awards bonus points toward your WCS score.
                        </p>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <Label htmlFor="ref-name" optional>Referee Full Name</Label>
                        <input id="ref-name" type="text" placeholder="e.g. Seun Adekunle" className={inputClass}
                          value={form.refereeName} onChange={(e) => set("refereeName", e.target.value)} />
                      </div>
                      <div>
                        <Label htmlFor="ref-company" optional>Referee Company / Organisation</Label>
                        <input id="ref-company" type="text" placeholder="e.g. Lagos Live Events Ltd" className={inputClass}
                          value={form.refereeCompany} onChange={(e) => set("refereeCompany", e.target.value)} />
                      </div>
                      <div>
                        <Label htmlFor="ref-contact" optional>Referee Phone / Email</Label>
                        <input id="ref-contact" type="text" placeholder="+234 800 000 0000" className={inputClass}
                          value={form.refereeContact} onChange={(e) => set("refereeContact", e.target.value)} />
                      </div>
                      <div>
                        <Label htmlFor="ref-rel" optional>Relationship to You</Label>
                        <input id="ref-rel" type="text" placeholder="e.g. Previous employer, collaborator" className={inputClass}
                          value={form.refereeRelationship} onChange={(e) => set("refereeRelationship", e.target.value)} />
                      </div>
                    </div>
                  ...
                  <section className="bg-surface-container-low rounded-2xl p-6 border border-outline-variant/10">
                    <div className="flex items-start justify-between gap-4 mb-4">
                      <div>
                        ...
                        <p className="text-sm text-on-surface-variant">
                          Sharing a portfolio or relevant certificate awards bonus points toward your WCS score.
                        </p>
                      </div>
                      <span className="shrink-0 text-xs font-bold text-amber-700 bg-amber-100 px-3 py-1.5 rounded-full">
                        +10 bonus pts
                    ...
                    <div>
                      <Label htmlFor="portfolio" optional>Paste links separated by commas</Label>
                      <textarea id="portfolio" rows={3}
                        placeholder="Google Drive, YouTube, LinkedIn, personal website..."
                        className="w-full px-4 py-3 bg-surface-container-highest border-none rounded-xl focus:ring-2 focus:ring-[#FF5A30] transition-all text-on-surface outline-none text-sm resize-none"
                        value={form.portfolioLinks} onChange={(e) => set("portfolioLinks", e.target.value)} />
                    </div>
                  ...
                  {/* Equipment */}
                  <div>
                    <Label htmlFor="equipment" optional>Equipment &amp; Tools You Own / Operate</Label>
              ...
              {step === 4 && (
                <div className="space-y-10">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="material-symbols-outlined text-[#FF5A30]">analytics</span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-surface-container-low rounded-2xl p-5 border border-outline-variant/10">
                      ...
                      <div className="space-y-3">
                        {[
                          { label: "Experience Level", pts: wcs.expPts, max: 35 },
                          { label: "Event Scale", pts: wcs.scalePts, max: 40 },
                          { label: "Touring Availability", pts: wcs.tourPts, max: 25 },
                        ].map((item) => (
                          <div key={item.label}>
                            <div className="flex justify-between text-xs mb-1">
                              <span className="text-on-surface-variant font-medium">{item.label}</span>
                              <span className="font-bold text-on-surface">{item.pts} / {item.max}</span>
                            </div>
                      ...
                      <div className="space-y-3">
                        {wcs.bonusItems.map((b) => (
                          <div key={b.label} className="flex items-start gap-3">
                            ...
                            >
                              {b.earned ? "check_circle" : "radio_button_unchecked"}
                            </span>
                            <div className="flex-1 min-w-0">
                              <p className={`text-xs font-semibold ${b.earned ? "text-on-surface" : "text-on-surface-variant/50"}`}>
                                {b.label}
                              </p>
                            </div>
                            <span className={`text-xs font-bold shrink-0 ${b.earned ? "text-amber-600" : "text-slate-300"}`}>
                              +{b.pts}
                            </span>
                          </div>
                        ))}
                  ...
                  {/* Tier card */}
                  <div className={`rounded-2xl p-6 border-2 ${tier.bg} ${tier.border}`}>
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div>
                        <p className={`text-xs font-bold uppercase tracking-wider ${tier.text} opacity-70`}>
                          Tier {tier.tier} · Score {tier.range}
                        </p>
                        <h4 className={`text-lg font-extrabold ${tier.text} font-(family-name:--font-manrope)`}>
                          {tier.label}
                        </h4>
                      </div>
                    </div>
                    <ul className="space-y-2 mb-4">
                      {tier.perks.map((p) => (
                        <li key={p} className={`flex items-start gap-2 text-sm ${tier.text}`}>
                          <span className="material-symbols-outlined text-sm mt-0.5 shrink-0" style={{ fontVariationSettings: "'FILL' 1" }}>
                            check_circle
                          </span>
                          {p}
                        </li>
                    ...
                    {tier.aya && (
                      <div className={`flex items-center gap-2 pt-4 border-t border-current/20 text-sm font-bold ${tier.text}`}>
                  ...
                  <details className="bg-surface-container-low rounded-2xl border border-outline-variant/10 overflow-hidden">
                    ...
                    <div className="px-6 pb-6 grid grid-cols-1 md:grid-cols-2 gap-3">
                      {TIER_CONFIG.map((t) => (
                        <div key={t.tier} className={`rounded-xl p-4 border ${t.bg} ${t.border} ${t.tier === tier.tier ? "ring-2 " + t.ring : ""}`}>
                          <div className="flex items-center justify-between mb-2">
                            <span className={`text-xs font-bold uppercase ${t.text}`}>Tier {t.tier}</span>
                            <span className={`text-xs font-semibold ${t.text} opacity-70`}>{t.range}</span>
                          </div>
                          <p className={`text-sm font-bold ${t.text} mb-1`}>{t.label}</p>
                          {t.aya && (
                            <p className={`text-xs ${t.text} opacity-70`}>Aya: {t.aya}</p>
                          )}
                        </div>
                      ))}
                    </div>
                  ...
                  <section className="bg-surface-container-low rounded-2xl p-6 border border-[#FF5A30]/10 space-y-5">
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="font-bold text-on-surface font-(family-name:--font-manrope)">Declaration &amp; Consent</h4>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                      <div>
                        ...
                        <input id="decl-name" type="text" placeholder={form.fullName || "Your full name"} className={inputClass} required
                          value={form.declarationName} onChange={(e) => set("declarationName", e.target.value)} />
                      </div>
                      <div>
                        <Label htmlFor="decl-date">Date</Label>
                        <input id="decl-date" type="date" className={inputClass} required
                          value={form.declarationDate} onChange={(e) => set("declarationDate", e.target.value)} />
                      </div>
                    </div>
                  </section>

                  <p className="text-xs text-center text-on-surface-variant">
                    Ckrowd Africa Technologies · CTaaS Workforce Registry · WCS v0.1
                    <br />
                    Enquiries: <span className="font-semibold">workforce@ckrowd.africa</span>
                  </p>
                </div>
              )}
              ...
              <footer className="flex items-center justify-between pt-8 mt-10 border-t border-outline-variant/10">
                ...
                <div className="flex gap-4">
                  ...
                  >
                    {step < STEPS.length - 1 ? "Continue" : "Submit Registration"}
              ...
              {[
                ...
                <div key={item.title} className="flex gap-4">
                  <div className="w-10 h-10 rounded-full bg-[#FF5A30]/10 flex items-center justify-center shrink-0">
                    <span className="material-symbols-outlined text-[#FF5A30] text-base">{item.icon}</span>
                  </div>
                  <div>
                    <h5 className="font-bold text-on-surface text-sm mb-0.5">{item.title}</h5>
