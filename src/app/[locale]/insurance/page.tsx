...
export default function InsurancePage() {
  ...
  const ECOSYSTEM_FLOW = [
    { step: "01", label: t('ecosystemFlow.0.label'), desc: t('ecosystemFlow.0.desc'), icon: "description" },
    { step: "02", label: t('ecosystemFlow.1.label'), desc: t('ecosystemFlow.1.desc'), icon: "fact_check" },
    { step: "03", label: t('ecosystemFlow.2.label'), desc: t('ecosystemFlow.2.desc'), icon: "school" },
    { step: "04", label: t('ecosystemFlow.3.label'), desc: t('ecosystemFlow.3.desc'), icon: "account_balance" },
    { step: "05", label: t('ecosystemFlow.4.label'), desc: t('ecosystemFlow.4.desc'), icon: "shield" },
    { step: "06", label: t('ecosystemFlow.5.label'), desc: t('ecosystemFlow.5.desc'), icon: "payments" },
  ];

  const INSURANCE_PRODUCTS = [
    { name: t('productSuite.0.name'), tag: t('productTags.event'), icon: "event_busy", who: ["promoter", "talent"], desc: t('productSuite.0.desc'), highlight: false },
    { name: t('productSuite.1.name'), tag: t('productTags.highest'), icon: "account_balance", who: ["promoter"], desc: t('productSuite.1.desc'), highlight: true },
    { name: t('productSuite.2.name'), tag: t('productTags.sme'), icon: "business", who: ["promoter", "vendor"], desc: t('productSuite.2.desc'), highlight: false },
    { name: t('productSuite.3.name'), tag: t('productTags.group'), icon: "groups", who: ["workforce"], desc: t('productSuite.3.desc'), highlight: false },
    { name: t('productSuite.4.name'), tag: t('productTags.specialty'), icon: "flight", who: ["promoter", "vendor"], desc: t('productSuite.4.desc'), highlight: false },
    { name: t('productSuite.5.name'), tag: t('productTags.embedded'), icon: "confirmation_number", who: ["promoter", "talent"], desc: t('productSuite.5.desc'), highlight: false },
  ];
  ...
  const STAKEHOLDERS = [
    {
      ...
      steps: (t.raw('stakeholders.promoter.steps') as Omit<Step, 'id'>[]).map((s, i) => ({ id: i + 1, ...s })),
      coverage: t.raw('stakeholders.promoter.coverage') as Coverage[],
      financeProducts: t.raw('stakeholders.promoter.financeProducts') as FinanceProduct[],
      color: "bg-[#FF5A30]",
      accent: "#FF5A30",
    },
    {
      ...
      steps: (t.raw('stakeholders.workforce.steps') as Omit<Step, 'id'>[]).map((s, i) => ({ id: i + 1, ...s })),
      coverage: t.raw('stakeholders.workforce.coverage') as Coverage[],
      financeProducts: t.raw('stakeholders.workforce.financeProducts') as FinanceProduct[],
      color: "bg-[#2D5A8E]",
      accent: "#2D5A8E",
    },
    {
      ...
      steps: (t.raw('stakeholders.vendor.steps') as Omit<Step, 'id'>[]).map((s, i) => ({ id: i + 1, ...s })),
      coverage: t.raw('stakeholders.vendor.coverage') as Coverage[],
      financeProducts: t.raw('stakeholders.vendor.financeProducts') as FinanceProduct[],
      color: "bg-[#3D6B2E]",
      accent: "#3D6B2E",
    },
    {
      ...
      steps: (t.raw('stakeholders.talent.steps') as Omit<Step, 'id'>[]).map((s, i) => ({ id: i + 1, ...s })),
      coverage: t.raw('stakeholders.talent.coverage') as Coverage[],
      financeProducts: t.raw('stakeholders.talent.financeProducts') as FinanceProduct[],
      color: "bg-[#7B3F8E]",
      accent: "#7B3F8E",
    },
  ];
      ...
      <main className="pt-24 pb-20 px-6 md:px-12 max-w-screen-2xl mx-auto flex flex-col gap-12">
        ...
        <header className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          <div className="lg:col-span-7">
            ...
            <h1 className="font-(family-name:--font-manrope) text-5xl md:text-6xl font-extrabold text-on-surface leading-tight tracking-tighter">
              {t.rich('hero.title', {
                spanHighlight: (chunks) => <span className="text-[#FF5A30]">{chunks}</span>,
                br: () => <br />
              })}
            </h1>
          ...
          <div className="lg:col-span-5 grid grid-cols-2 gap-4">
            {[
              { value: t('stats.values.portfolio'), label: t('stats.portfolio') },
        ...
        {activeTab === "overview" && (
          <div className="flex flex-col gap-16">
            ...
            {/* Pipeline flow */}
            <section>
              <h2 className="font-(family-name:--font-manrope) text-2xl font-bold mb-8">{t('overview.pipelineTitle')}</h2>
            ...
            {/* Stakeholder cards */}
            <section>
              <h2 className="font-(family-name:--font-manrope) text-2xl font-bold mb-8">{t('overview.stakeholdersTitle')}</h2>
                ...
                {STAKEHOLDERS.map(s => (
                  ...
                  >
                    ...
                    <div className="p-6 flex flex-col gap-4 flex-1">
                      ...
                      <div>
                        <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-2">{t('profile.coverageTitle')}</p>
            ...
            <section>
              <h2 className="font-(family-name:--font-manrope) text-2xl font-bold mb-8">{t('overview.productsTitle')}</h2>
            ...
            <div className="bg-linear-to-br from-[#FF5A30] to-[#cc4826] rounded-2xl p-10 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
              <div className="relative z-10">
                <h3 className="font-(family-name:--font-manrope) text-2xl font-bold text-white mb-2">{t('overview.cta.title')}</h3>
                <p className="text-white/80 text-sm">{t('overview.cta.description')}</p>
              </div>
              <button
                onClick={() => setActiveTab("onboarding")}
        ...
        {activeTab === "onboarding" && (
          <div className="flex flex-col gap-10">
            ...
            {/* Role selector */}
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-3">{t('onboarding.selectRole')}</p>
              ...
              <div className="lg:col-span-4 flex flex-col gap-4">
                <div className="bg-surface-container-lowest rounded-2xl overflow-hidden border border-outline-variant/10 shadow-sm">
                  <div className={`${current.color} p-5`}>
                    ...
                    <p className="font-(family-name:--font-manrope) font-bold text-white">{current.label} — {t('tabs.onboarding')}</p>
                  </div>
                    ...
                    {current.steps.map((step, i) => (
                      ...
                      >
                        ...
                        <div className="pl-6">
                          ...
                          <p className="text-xs text-on-surface-variant">{t('onboarding.step', { current: step.id, total: current.steps.length })}</p>
                        </div>
                      </button>
                    ))}
                  </div>
                ...
                <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/10 shadow-sm overflow-hidden">
                  <div className="px-5 py-4 border-b border-outline-variant/10">
                    <p className="font-(family-name:--font-manrope) font-bold text-sm">{t('onboarding.partners')}</p>
                  </div>
                  {(t.raw('onboarding.partnersList') as { name: string, sub: string }[]).map((p, i) => (
                  ...
                  <div key={step.id} className="flex flex-col gap-4">
                    <div className="bg-surface-container-lowest rounded-2xl p-8 border border-outline-variant/10 shadow-sm">
                      <div className="flex items-start justify-between gap-4 mb-6">
                        <div>
                          <span className="text-[10px] font-black uppercase tracking-widest text-[#FF5A30] block mb-2">{t('onboarding.step', { current: step.id, total: current.steps.length })}</span>
                      ...
                      {step.id === 4 && (
                        <div className="bg-green-50 border border-green-200 rounded-xl p-5 mb-6 flex gap-4 items-start">
                          ...
                          <div>
                            <p className="font-(family-name:--font-manrope) font-bold text-green-800 text-sm mb-1">{t('onboarding.activates.title')}</p>
                            <p className="text-xs text-green-700 leading-relaxed">{t('onboarding.activates.description')}</p>
                          </div>
                        </div>
                      )}
                      ...
                      {step.id === 5 && (
                        <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 mb-6 flex gap-4 items-start">
                          ...
                          <div>
                            <p className="font-(family-name:--font-manrope) font-bold text-amber-800 text-sm mb-1">{t('onboarding.finance.title')}</p>
                            <p className="text-xs text-amber-700 leading-relaxed">{t('onboarding.finance.description')}</p>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="flex gap-3">
                        ...
                        {activeStep < current.steps.length && (
                          <button type="submit" className="px-6 py-2.5 rounded-xl bg-[#FF5A30] text-white text-sm font-bold hover:opacity-90 transition-all shadow-lg shadow-[#FF5A30]/20">
                            {t('onboarding.actions.continue', { next: activeStep + 1 })}
                          </button>
                        )}
                        {activeStep === current.steps.length && (
                    ...
                    <div className="bg-surface-container-lowest rounded-2xl p-5 border border-outline-variant/10 shadow-sm">
                      <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-3">{t('onboarding.progress')}</p>
                      <div className="flex gap-1.5 mb-2">
                        {current.steps.map((s) => (
                          <div key={s.id} className={`flex-1 h-1.5 rounded-full transition-all ${s.id <= activeStep ? "bg-[#FF5A30]" : "bg-outline-variant/30"}`} />
                        ))}
                      </div>
                      <p className="text-xs text-on-surface-variant">{t('onboarding.progressDetail', { current: activeStep, total: current.steps.length })}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* FAQ */}
            <section>
              <h2 className="font-(family-name:--font-manrope) text-2xl font-bold mb-6">{t('faq.title')}</h2>
        ...
        {activeTab === "products" && (
          <div className="flex flex-col gap-12">
              ...
              {INSURANCE_PRODUCTS.map((p, i) => (
                <div key={i} className="bg-surface-container-lowest rounded-2xl overflow-hidden border border-outline-variant/10 shadow-sm hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="p-6 flex flex-col gap-4">
                    ...
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-2">{t('onboarding.selectRole')}</p>
            ...
            <div className="bg-linear-to-br from-on-surface to-on-surface/80 rounded-2xl p-10 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
              <div className="relative z-10">
                <span className="text-[10px] font-black uppercase tracking-widest text-[#FF5A30] block mb-2">{t('products.underwriter.title')}</span>
                <h3 className="font-(family-name:--font-manrope) text-2xl font-bold text-white mb-2">{t('products.underwriter.brand')}</h3>
                <p className="text-white/60 text-sm max-w-md">{t('products.underwriter.description')}</p>
              </div>
              <div className="relative z-10 text-right shrink-0">
                <p className="text-[10px] font-bold uppercase tracking-widest text-white/40 mb-1">{t('products.underwriter.portfolio')}</p>
                <p className="font-(family-name:--font-manrope) text-5xl font-black text-[#FF5A30] leading-none">{t('products.underwriter.portfolioValue')}</p>
                <p className="text-xs text-white/40 mt-1">{t('products.underwriter.scale')}</p>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-5 flex flex-col gap-5">
                ...
                <div className="bg-surface-container-lowest rounded-2xl p-6 border border-outline-variant/10 shadow-sm">
                  <h3 className="font-(family-name:--font-manrope) font-bold mb-5">{t('profile.certification')}</h3>
                        ...
                        <div className="flex-1 pl-6">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${i === 0 ? "bg-green-100 text-green-700" : "bg-surface-container text-on-surface-variant"}`}>
                              {i === 0 ? t('profile.status.complete') : t('profile.status.pending')}
                            </span>
                          </div>
                ...
                {/* Finance products */}
                <div className="bg-surface-container-lowest rounded-2xl p-6 border border-outline-variant/10 shadow-sm">
                  <h3 className="font-(family-name:--font-manrope) font-bold mb-5">{t('profile.financeTitle')}</h3>
                    ...
                    {current.financeProducts.map((fp, i) => (
                      <div key={i} className="border border-outline-variant/20 rounded-xl p-5">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <span className="text-[9px] font-black uppercase tracking-widest bg-surface-container text-on-surface-variant px-2.5 py-1 rounded-full">{t('profile.bankName')}</span>
                        </div>
                        <div className="grid grid-cols-3 gap-3 pt-3 border-t border-outline-variant/20">
                          ...
                          ] as [string, string][]).map(([label, val]: [string, string]) => (
                            <div key={label}>
              ...
              <div className="lg:col-span-7 flex flex-col gap-5">
                {/* Coverage detail */}
                <div className="bg-surface-container-lowest rounded-2xl p-6 border border-outline-variant/10 shadow-sm">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-6">{t('profile.coverageTitle')}</p>
                ...
                <div className="bg-linear-to-br from-on-surface to-on-surface/80 rounded-2xl p-8 relative overflow-hidden">
                  <div className="relative z-10">
                    <span className="text-[10px] font-black uppercase tracking-widest text-[#FF5A30] block mb-2">{t('profile.status.pending')}</span>
                    <h3 className="font-(family-name:--font-manrope) text-xl font-bold text-white mb-2">{t('profile.cta.title')}</h3>
                    <p className="text-white/60 text-sm mb-6">{t('profile.cta.description')}</p>
                    <button
                      onClick={() => setActiveTab("onboarding")}
