...
export default function ArtmgmtPage() {
	...
	const createMutation = useMutation({
		mutationFn: (payload: Parameters<typeof createRosterArtist>[0]) => createRosterArtist(payload),
		onSuccess: (result) => {
			if (result.success) {
									...
									>
										<div className="flex items-start gap-3">
											...
											<div className="flex-1 min-w-0">
												<p className="font-bold text-slate-900 truncate">{String(artist.name)}</p>
										...
										{Boolean(artist.bio) && (
											<p className="text-xs text-slate-500 line-clamp-2">{String(artist.bio)}</p>
										)}
							...
							<div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
								...
								<form onSubmit={handleSubmit} className="flex flex-col gap-4">
									...
									<div className="grid grid-cols-2 gap-3">
										...
										<div>
											...
											<input
												type="text"
												value={form.spotify}
												onChange={(e) => setForm((p) => ({ ...p, spotify: e.target.value }))}
									...
									<label className="flex items-center gap-3 cursor-pointer">
										...
										<span className="text-sm font-medium text-slate-700">{t("form.isActive")}</span>
									</label>
