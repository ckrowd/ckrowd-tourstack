...
export default function EoiDocumentsPanel({ eoiId }: { eoiId: string }) {
	...
	if (docs.length === 0) {
		return (
			<p className="text-xs text-on-surface-variant/50 mt-3 italic">{t("noDocsYet")}</p>
		);
	}
	...
	return (
		<div className="mt-3 space-y-2">
			{docs.map((doc) => (
				<div
					key={doc.id}
					className="flex items-center gap-3 rounded-xl border border-outline-variant/30 bg-surface-container-low px-3 py-2.5"
				>
					...
					<a
						...
						className="inline-flex items-center gap-1 rounded-full border border-outline-variant/30 px-2.5 py-1 text-[10px] font-semibold text-on-surface-variant hover:bg-surface-container transition shrink-0"
					>
