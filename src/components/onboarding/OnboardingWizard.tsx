...
export default function OnboardingWizard() {
	...
	return (
		<div className="min-h-[100dvh] bg-[#0a0a0a] text-white font-(family-name:--font-geist) lg:grid lg:grid-cols-[340px_1fr]">
			{/* Left rail (desktop) */}
			<aside className="relative hidden lg:flex flex-col justify-between border-r border-white/8 px-9 py-10 overflow-hidden">
				...
				<div className="relative z-10">
					...
					<p className="mt-3 font-(family-name:--font-geist-mono) text-[11px] uppercase tracking-[0.16em] text-white/40">
			...
			<div className="flex flex-col min-h-[100dvh] lg:min-h-0">
				...
				<div className="lg:hidden px-5 pt-6 pb-4">
					<div className="flex items-center justify-between mb-4">
						...
						<span className="font-(family-name:--font-geist-mono) text-[11px] uppercase tracking-[0.14em] text-white/40">
							{t("progress", { current: step + 1, total })}
						</span>
					</div>
					<div className="h-1 w-full rounded-full bg-white/10 overflow-hidden">
				...
				<main className="flex-1 flex flex-col px-5 sm:px-8 md:px-14 py-8 md:py-14">
					<div key={current} className="funnel-step-in flex-1 w-full max-w-2xl mx-auto lg:mx-0">
						...
						<p className="mt-3 text-white/55 text-sm md:text-base max-w-xl">
							{t(`${current}.subtitle`)}
						</p>
											...
											>
												<span
													className={`flex h-11 w-11 items-center justify-center rounded-xl transition-colors ${
														selected ? "bg-orange text-white" : "bg-white/[0.06] text-white/70"
													}`}
												>
												...
												<span>
													<span className="block font-semibold text-[15px] text-white">
														{r.title}
													</span>
											...
											>
												<span
													className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-md border transition-colors ${
														selected ? "border-orange bg-orange" : "border-white/25"
													}`}
												>
													{selected && (
							...
							{current === "review" && (
								<div className="space-y-3">
									...
									<ReviewRow
										...
										editLabel={t("review.edit")}
									/>
									<p className="pt-3 text-xs text-white/40 leading-relaxed">
										{t("review.finePrint")}
									</p>
								</div>
							)}
					...
					{/* Footer nav */}
					<div className="mt-10 w-full max-w-2xl mx-auto lg:mx-0 flex items-center justify-between gap-4 border-t border-white/8 pt-6">
						{step === 0 ? (
							<Link
								href="/"
...
function ReviewRow({
	...
	return (
		<div className="flex items-start justify-between gap-4 rounded-xl border border-white/10 bg-white/[0.02] px-5 py-4">
			<div className="min-w-0">
				<p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-white/40">{label}</p>
				<p className="mt-1 text-sm text-white/85 break-words">{value}</p>
			</div>
			<button
				type="button"
				onClick={onEdit}
